...
impl NodeIdentity {
    ...
    /// Create an identity from a hex-encoded 32-byte secret key.
    ///
    /// Used to adopt a key generated by Reality's keytool (`keytool export`), so
    /// the same key that deploys the rApp also signs its state-channel snapshots
    /// (the testnet only accepts snapshots signed by the deployed rApp's key).
    pub fn from_secret_key_hex(hex_key: &str) -> Result<Self> {
        let bytes = hex::decode(hex_key.trim()).context("Invalid identity key hex")?;
